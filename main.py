...
# 対応モデル一覧 https://github.com/openai/tiktoken/blob/main/tiktoken/model.py#L7
try:
    ...
    tiktoken_enc = tiktoken.encoding_for_model(DEFAULT_MODEL)
...
def chatComplitionDiffs(changes):
    ...
    try:
        completions = openai.ChatCompletion.create(
                ...
                # NotSupport o3-mini temperature=0.2, # TODO: o3-miniの時だけ外す
                stream=False,
                messages=messages
            )
        answer = completions.choices[0].message["content"].strip()
        answer += "\n\nThis comment was generated by an artificial intelligence duck."
        answer += f"\nPrompt:{completions.usage.prompt_tokens} Completion:{completions.usage.completion_tokens} Total:{completions.usage.total_tokens} Model:{completions.model}"
    except Exception as e:
        logger.error(e)
